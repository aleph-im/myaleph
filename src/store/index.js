...
import {
  get_nuls_balance_info,
  get_ethereum_balance_info,
  get_neo_balance_info
  ...
  const Store = new Vuex.Store({
    ...
    mutations: {
      ...
      set_pages(state, pages) {
        // TODO: handle per-page mutation
        state.pages = pages;
      },
      set_files(state, files) {
        // TODO: handle per-files mutation
        state.files = files;
      },
    ...
    actions: {
      ...
      async update_balance({ state, commit }) {
        if (state.account !== null) {
          if (state.account.type === "NULS2") {
            ...
            commit(
              "set_balance_info",
              await get_ethereum_balance_info(
                state.account.address,
      ...
      async create_new_folders({ state, commit, dispatch }, account) {
        const create_new_folder = async folder_name => {
          let post_content = {
          ...
          await encrypt_content_for_self(post_content, ["filename"], account);
          ...
          return commit("add_file", msg);
        };

        try {
          await Promise.all(
            config.folder_names_to_create.map(create_new_folder)
          );
          console.log(folder_names_to_create);
          console.info("all created!");
        } catch (e) {
          console.error(e);
          console.warn("Can't create new folders for the account");
        }
        await dispatch("update_files");
      }
